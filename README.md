...
* **Fast Context Switching:** A full context switch (from PendSV trigger to handler exit) takes just 3.5µs on average at 133MHz.
  -  The context switch routine includes quite a few instructions in order to handle the ARMv6-M's inability to push/pop upper registers. There might be a method to reduce this. More research is required.
...
When running two instances of the Dhrystone benchmark, this RTOS achieved a **1.85x speedup** when utilizing both cores compared to single-core execution (1.52s vs 2.81s). 
